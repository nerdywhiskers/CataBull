...
3. **search_queries**: WebSearch queries used by the agent's Deep Scan path against aggregator job boards (LinkedIn, Wellfound, RemoteOK, etc.). Greenhouse / Ashby / Lever / Workday / BambooHR / Teamtailor are hit through their direct APIs from `tracked_companies` — not via search queries.
4. **tracked_companies**: Companies to scan via direct ATS APIs (zero LLM tokens). Each entry can carry a `health:` block (W4 auto-recovery state) and `auto_disabled` flag.
