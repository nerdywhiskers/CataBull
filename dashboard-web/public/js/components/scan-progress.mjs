...
export function deepProgressFromEvent(data = {}) {
  ...
  if (stage === 'l4:detect') return { visible: true, tone: 'running', eyebrow: 'Deep Scan · Level 4', title: 'Preparing market discovery…', detail: 'Checking JobSpy plus API/RSS provider support.', meta: '' };
  ...
  if (stage === 'l4:start') return { visible: true, tone: 'running', eyebrow: 'Deep Scan · Level 4', title: `Running ${Number(data.providers) || 0} market providers`, detail: `${Number(data.queries) || 0} query terms across JobSpy + public feeds`, meta: '' };
  if (stage === 'l4:provider:start') return { visible: true, tone: 'running', eyebrow: 'Deep Scan · Level 4', title: `${data.provider || 'Provider'} start`, detail: data.runner ? `Runner: ${data.runner}` : `Querying ${Number(data.totalQueries) || 0} search terms`, meta: '' };
  if (stage === 'l4:provider:done') return { visible: true, tone: 'running', eyebrow: 'Deep Scan · Level 4', title: `${data.provider || 'Provider'} done`, detail: `${Number(data.hits) || 0} hits`, meta: '' };
  if (stage === 'l4:provider:skip') return { visible: true, tone: 'running', eyebrow: 'Deep Scan · Level 4', title: `${data.provider || 'Provider'} skipped`, detail: data.reason || 'Not available', meta: '' };
  if (stage === 'l4:provider:error') return { visible: true, tone: 'running', eyebrow: 'Deep Scan · Level 4', title: `${data.provider || 'Provider'} error`, detail: data.error || 'Provider failed', meta: data.query || '' };
  if (stage === 'l4:query:start') return { visible: true, tone: 'running', eyebrow: 'Deep Scan · Level 4', title: `${data.provider || 'Provider'} · ${data.query || 'query'}`, detail: `Query ${Number(data.queryIndex) + 1}/${Number(data.total) || 0}`, meta: data.site ? `Site: ${data.site}` : '' };
  if (stage === 'l4:query:done') return { visible: true, tone: 'running', eyebrow: 'Deep Scan · Level 4', title: `${data.provider || 'Provider'} · ${data.query || 'query'} done`, detail: `${Number(data.hits) || 0} hits`, meta: data.site ? `Site: ${data.site}` : '' };
  ...
  if (stage === 'l4:filtered') return { visible: true, tone: 'running', eyebrow: 'Deep Scan · Level 4', title: 'Filtering market hits', detail: `${Number(data.kept) || 0} roles still in play`, meta: '' };
