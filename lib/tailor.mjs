...
import { canonicalCompanyName, canonicalRoleName } from './role-identity.mjs';
...
export function tailorSlug(company, role, { date = new Date().toISOString().slice(0, 10) } = {}) {
  const norm = (s) => String(s || '')
    ...
    .replace(/^-+|-+$/g, '')
    .slice(0, 40);
  const c = norm(canonicalCompanyName(company)) || 'unknown-company';
  const r = norm(canonicalRoleName(role)) || 'unknown-role';
  return `${c}-${r}-${date}`;
}
...
export function writeTailorBundle(rootOrWorkspace, slug, payload, { company, role } = {}) {
  ...
  const cvDocPath = `${dir}/cv.doc`;
  const clDocPath = `${dir}/cover-letter.doc`;
  ...
  const cvHtml = renderTailorMarkdownHtml(payload.tailored_cv_markdown, { title: `${company || 'Tailored'} CV` });
  const coverLetterHtml = renderTailorMarkdownHtml(payload.cover_letter_markdown, { title: `${company || 'Tailored'} Cover Letter` });
  ws.write(cvHtmlPath, cvHtml);
  ws.write(clHtmlPath, coverLetterHtml);
  ws.write(cvDocPath, cvHtml);
  ws.write(clDocPath, coverLetterHtml);
...
export function writeTailorReport(rootOrWorkspace, result, { company, role, url, date = new Date().toISOString().slice(0, 10) } = {}) {
  const ws = asWorkspace(rootOrWorkspace);
  let candidate = parseInt(nextReportNumber(ws), 10);
  while (true) {
    const number = String(candidate).padStart(3, '0');
    ...
    if (ws.writeExclusive(path, markdown)) return { filename, path, number };
    candidate++;
  }
}
