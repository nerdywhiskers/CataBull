...
/** Replace one file atomically and clean the sibling temporary file on failure. */
export function atomicWriteFileSync(absPath, content, {
  writeFile = writeFileSync,
  renameFile = renameSync,
  removeFile = unlinkSync,
} = {}) {
  const tempPath = `${absPath}.catabull-tmp-${process.pid}-${++atomicWriteCounter}`;
  try {
    writeFile(tempPath, content);
    renameFile(tempPath, absPath);
  } catch (error) {
    try {
      if (existsSync(tempPath)) removeFile(tempPath);
...
export class LocalWorkspace {
  ...
  /** Atomically replace a file. Creates parent directories as needed. */
