...
const DEFAULT_REPO = 'your-github-user/careerbot';
...
export async function getUpdateStatus(packageRoot, opts = {}) {
  ...
  if (installKind === 'unsupported') {
    return {
      ...
      canGitPull: false,
      status: 'unsupported',
      message: 'CareerBot install path not recognized. Reinstall via the official one-line installer to enable in-app updates.',
    };
  }
  ...
  return {
    ...
    message: buildGitMessage({ updateAvailable, dirty, onTargetBranch, branch, remoteFetchError, localVersion, remoteVersion }),
  };
}
...
export async function applyUpdate(packageRoot, opts = {}) {
  ...
  return {
    ...
    needsRestart: true,
    message: 'CareerBot updated via npm. Restart the dashboard to use the new version.',
  };
}
...
export async function applyGitPull(packageRoot, opts = {}) {
  ...
  return {
    ...
    message: before.localCommit === after.localCommit
      ? 'Already up to date.'
      : 'CareerBot updated via git pull. Restart the dashboard to use the new version.',
  };
}
...
export function detectInstallKind(packageRoot) {
  ...
  // npm-global layout: <prefix>/lib/node_modules/careerbot on POSIX,
  // <prefix>\node_modules\careerbot on Windows. The basename is the package
  ...
  if (parent === 'node_modules' && self === 'careerbot') return 'npm-global';
  return 'unsupported';
}
...
function buildGitMessage({ updateAvailable, dirty, onTargetBranch, branch, remoteFetchError, localVersion, remoteVersion }) {
  if (!updateAvailable) {
    if (remoteFetchError) return `Could not check GitHub: ${remoteFetchError}`;
    return 'CareerBot is up to date.';
  }
  const versionNote = (localVersion && remoteVersion && localVersion !== remoteVersion)
    ? `${localVersion} → ${remoteVersion}`
    : 'new commits available';
  if (!onTargetBranch) return `Update available (${versionNote}), but you're on branch ${branch || 'detached'}. Use npm Update or switch branches.`;
...
// are ignored — CareerBot ships plain x.y.z so this is sufficient. Returns
