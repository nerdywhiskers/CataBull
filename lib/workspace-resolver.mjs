...
import { existsSync, mkdirSync, copyFileSync, readdirSync, cpSync, readFileSync } from 'fs';
...
export function readGlobalWorkspacePreference({ home = homedir(), envText = null } = {}) {
  ...
  for (const rawLine of text.split(/\r?\n/)) {
    ...
    return normalizeGlobalWorkspacePreference(value);
  }

  return 'home';
}

function detectInstallKind(packageRoot) {
...
export function resolveWorkspaceRoot(opts = {}) {
  ...
  if (installKind === 'npm-global') {
    const preference = normalizeGlobalWorkspacePreference(
      globalWorkspacePreference || readGlobalWorkspacePreference({ home })
    );
    if (preference === 'cwd' && looksLikeWorkspace(cwd)) {
      return { root: resolve(cwd), reason: 'cwd', created: false, preference };
    }
    ...
    return { root: resolve(homeWorkspace), reason: 'home', created, preference };
  }
