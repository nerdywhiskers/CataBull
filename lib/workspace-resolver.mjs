...
import { existsSync, mkdirSync, copyFileSync, readdirSync, cpSync, readFileSync, writeFileSync } from 'fs';
...
export const GLOBAL_WORKSPACE_PREFERENCE_KEY = 'CATABULL_GLOBAL_WORKSPACE_PREFERENCE';
const GLOBAL_WORKSPACE_PREFERENCES = new Set(['home', 'cwd']);

export const RESOLUTION_REASONS = ['env', 'cwd', 'home', 'project'];

export function homeWorkspaceRoot(home = homedir()) {
  return join(home, HOME_WORKSPACE_DIR);
}

export function isHermesProfileHome(home) {
  const normalized = String(home || '').replace(/\\/g, '/');
  return /\/\.hermes\/profiles\/[^/]+\/home$/.test(normalized);
}

export function resolveEffectiveHome({ home = homedir(), userHome = null } = {}) {
  const requested = resolve(String(home || homedir()));
  const actualUserHome = resolve(String(userHome || userInfo().homedir || requested));
  if (isHermesProfileHome(requested) && actualUserHome !== requested) return actualUserHome;
  return requested;
}

export function globalPreferenceEnvPath(home = homedir()) {
  return join(homeWorkspaceRoot(home), '.env');
}

export function normalizeGlobalWorkspacePreference(value) {
  const normalized = String(value || '').trim().toLowerCase();
  return GLOBAL_WORKSPACE_PREFERENCES.has(normalized) ? normalized : 'home';
}

export function readGlobalWorkspacePreference({ home = homedir(), envText = null } = {}) {
  const text = envText == null
    ? (existsSync(globalPreferenceEnvPath(home)) ? readFileSync(globalPreferenceEnvPath(home), 'utf-8') : '')
    : String(envText || '');
  ...
  for (const rawLine of text.split(/\r?\n/)) {
    ...
    if (eq === -1) continue;
    if (line.slice(0, eq).trim() !== GLOBAL_WORKSPACE_PREFERENCE_KEY) continue;
    let value = line.slice(eq + 1).trim();
    if ((value.startsWith('"') && value.endsWith('"')) || (value.startsWith("'") && value.endsWith("'"))) {
      value = value.slice(1, -1);
    }
    return {
      preference: normalizeGlobalWorkspacePreference(value),
      persisted: true,
    };
  }
...
export function writeGlobalWorkspacePreference(preference, { home = homedir() } = {}) {
  const normalized = normalizeGlobalWorkspacePreference(preference);
  const envPath = globalPreferenceEnvPath(home);
  ...
  for (const rawLine of lines) {
    ...
    if (key !== GLOBAL_WORKSPACE_PREFERENCE_KEY) {
      out.push(rawLine);
      continue;
    }
    out.push(`${GLOBAL_WORKSPACE_PREFERENCE_KEY}=${normalized}`);
    wrote = true;
  }

  if (!wrote) out.push(`${GLOBAL_WORKSPACE_PREFERENCE_KEY}=${normalized}`);
  mkdirSync(homeWorkspaceRoot(home), { recursive: true });
  writeFileSync(envPath, `${out.filter((line, index, arr) => !(line === '' && index === arr.length - 1)).join('\n')}\n`, 'utf-8');
  return normalized;
}
...
export function resolveWorkspaceRoot(opts = {}) {
  const {
    ...
    globalWorkspacePreference = null,
  } = opts;

  const effectiveHome = resolveEffectiveHome({ home, userHome });

  const envRoot = env.CATABULL_WORKSPACE_ROOT;
  if (envRoot && envRoot.trim()) {
    return { root: resolve(envRoot.trim()), reason: 'env', created: false, preference: 'home' };
  }

  if (installKind === 'npm-global') {
    const pref = globalWorkspacePreference
      ? { preference: normalizeGlobalWorkspacePreference(globalWorkspacePreference), persisted: true }
      : readGlobalWorkspacePreference({ home: effectiveHome });
    const { preference, persisted } = pref;
    const cwdIsWorkspace = looksLikeWorkspace(cwd);
    if (preference === 'cwd' && cwdIsWorkspace) {
      return {
        root: resolve(cwd),
        reason: 'cwd',
        created: false,
        ...
        shouldPromptForWorkspacePreference: false,
      };
    }

    const homeWorkspace = homeWorkspaceRoot(effectiveHome);
    let created = false;
    ...
    return {
      root: resolve(homeWorkspace),
      reason: 'home',
      created,
      preference,
      preferencePersisted: persisted,
      shouldPromptForWorkspacePreference: !persisted && cwdIsWorkspace,
    };
  }

  if (looksLikeWorkspace(cwd)) {
    return { root: resolve(cwd), reason: 'cwd', created: false, preference: 'home' };
  }

  if (allowProjectFallback && projectRoot && looksLikeWorkspace(projectRoot)) {
    return { root: resolve(projectRoot), reason: 'project', created: false, preference: 'home' };
  }

  // Fresh-install path. Land the user in ~/.catabull/, creating the
  // directory on first run.
  const normalizedHomeWorkspace = homeWorkspaceRoot(effectiveHome);
  let created = false;
  if (autoCreate && !existsSync(normalizedHomeWorkspace)) {
    mkdirSync(normalizedHomeWorkspace, { recursive: true });
    created = true;
  }
  return { root: resolve(normalizedHomeWorkspace), reason: 'home', created, preference: 'home' };
}
...
const SYSTEM_SYNC_WRAPPERS = [
  { workspaceRel: 'merge-tracker.mjs', packageRel: 'scripts/merge-tracker.mjs' },
  { workspaceRel: 'generate-pdf.mjs', packageRel: 'scripts/generate-pdf.mjs' },
];
...
export function syncSystemLayer(packageRoot, workspaceRoot) {
  ...
  const writeWrapper = (workspaceRel, packageRel) => {
    const src = join(pkg, packageRel);
    if (!existsSync(src)) return;
    const dest = join(ws, workspaceRel);
    mkdirSync(dirname(dest), { recursive: true });
    const sourceUrl = pathToFileURL(src).href;
    const body = [
      '#!/usr/bin/env node',
      `process.env.CATABULL_WORKSPACE_ROOT = process.env.CATABULL_WORKSPACE_ROOT || ${JSON.stringify(ws)};`,
      `await import(${JSON.stringify(sourceUrl)});`,
      '',
    ].join('\n');
    writeFileSync(dest, body, 'utf8');
    synced.push(workspaceRel);
  };

  for (const rel of SYSTEM_SYNC_FILES) copyOne(rel);
  for (const wrapper of SYSTEM_SYNC_WRAPPERS) writeWrapper(wrapper.workspaceRel, wrapper.packageRel);
...
export function ensureWorkspace(opts = {}) {
  ...
  const inferredInstallKind = resolverOpts.installKind || detectInstallKind(packageRoot);
  ...
  return { ...resolution, scaffold, installKind: inferredInstallKind };
}
