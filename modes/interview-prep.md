...
Before assigning `Fit`, query `memory/interview-feedback.md` for active entries tied to the same story or topic. If a story has repeated feedback that it "didn't land", downgrade it from `strong` to `partial` unless you have newer contradictory evidence.
...
4. After the interview happens, ask for quick story-level feedback so you can upsert one entry into `memory/interview-feedback.md` about what landed or missed
