...
// --- detectInstallKind ---
{
  const dir = mkdtempSync(join(tmpdir(), 'careerbot-update-detect-'));
  writeFileSync(join(dir, 'package.json'), JSON.stringify({ version: '1.0.0' }));
  assert.equal(detectInstallKind(dir), 'unsupported', 'plain dir w/ package.json is unsupported');

  // Fake an npm-global layout: <prefix>/node_modules/careerbot/package.json
  const nmRoot = mkdtempSync(join(tmpdir(), 'careerbot-update-nm-'));
  const nmInstall = join(nmRoot, 'node_modules', 'careerbot');
  mkdirSync(nmInstall, { recursive: true });
  writeFileSync(join(nmInstall, 'package.json'), JSON.stringify({ version: '1.0.0' }));
  assert.equal(detectInstallKind(nmInstall), 'npm-global', 'node_modules/careerbot layout = npm-global');

  // Git checkout: has .git directory.
  const gitDir = mkdtempSync(join(tmpdir(), 'careerbot-update-git-'));
...
{
  const dir = mkdtempSync(join(tmpdir(), 'careerbot-update-unsupported-'));
...
{
  const nmRoot = mkdtempSync(join(tmpdir(), 'careerbot-update-nm-status-'));
  const install = join(nmRoot, 'node_modules', 'careerbot');
  ...
  const status = await getUpdateStatus(install, { repo: 'invalid-owner/__careerbot_test__no_such_repo__', httpTimeoutMs: 2_000 });
...
{
  const remote = mkdtempSync(join(tmpdir(), 'careerbot-update-remote-'));
  run('git', ['init', '--bare'], remote);

  const seed = mkdtempSync(join(tmpdir(), 'careerbot-update-seed-'));
  ...
  const local = mkdtempSync(join(tmpdir(), 'careerbot-update-local-'));
  ...
  const opts = { repo: 'invalid-owner/__careerbot_test_git__', httpTimeoutMs: 2_000 };
